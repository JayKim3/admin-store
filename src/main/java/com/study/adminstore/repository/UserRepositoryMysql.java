...
//            final PreparedStatement ps = connection.prepareStatement("insert into user(account, password, status, email, phone_number, registered_at, unregistered_at, created_at, created_by, updated_at, updated_by) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", Statement.RETURN_GENERATED_KEYS);
...
//        jdbcTemplate.update(connection -> {
//            final PreparedStatement ps = connection.prepareStatement("update user set account = ?, password = ?, status =?, email =?, phone_number = ?, registered_at = ?, unregistered_at = ? where id = ?");
//            ps.setString(1, user.getAccount());
//            ps.setString(2, user.getPassword());
//            ps.setString(3, user.getStatus());
//            ps.setString(4, user.getEmail());
